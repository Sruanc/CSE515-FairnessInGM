...
import igraph as ig
import warnings
from pgmpy.estimators import PC, HillClimbSearch, ExhaustiveSearch, ExpertKnowledge
...
def _call_pgmpy(data, sensitive_attributes, verbose=True, threshold=0.33):
    ...
    for max_iter in [1, 5, 10, 50, 100, 200, 500, 1000, 5000, 10000]:
        ...
        # Create ExpertKnowledge for pgmpy 1.0.0+ API
        expert_knowledge = ExpertKnowledge(
            required_edges=fixed_edges if fixed_edges else None,
            forbidden_edges=black_list if black_list else None
        )
